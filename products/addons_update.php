...
if($action=='remove'){
    $sql = "DELETE FROM `addon` WHERE `addon_No`='".$id."'";


        if ($conn->query($sql) === TRUE){
            echo '<script> location.replace("addons.php"); </script>';
...
<div class="art-layout-wrapper">
	...
	if($addon=='' || $price==""){
		...
		$sql = "UPDATE `addon` SET `addon_Type`='".$addon."',`addon_Price`='".$price."' WHERE `addon_No`='".$id."'
	...
	";
		...
		<a href="addons.php? id='.$id.'"  class="art-button" >Done</a></center>';
			...
			echo mysql_errno($conn) . ": " . mysql_error($conn) . "\n";
					...
					$sql = "SELECT * FROM `addon` WHERE `addon_No`='".$id."'";
 $res = mysqli_query($conn,$sql);
		...
		<tr style="background-color: black; color: white;"><td style="width: 50%; ">Addon Type</td><td style="width: 25%; ">Price</td><td style="width: 25%; ">Action</td></tr>
					...
					<form name="form" method="post" style="margin-left: 5%">
		<tr><td style="width: 50%; "><input name="addon" value="'.$row['addon_Type'].'" type="text"><br></td>
		<td style="width: 25%; "><input name="price" value="'.$row['addon_Price'].'" type="number"><br></td>
		<td style="width: 25%; "><input type="submit" width="100px" name="submit" value="Update" class="art-button">
				</td></tr>
		</form>
